...
void RemoteControlProcess(Remote *rc)
{
	...
	#ifdef STEER	
		pwmwatch[0]=(rc->ch0 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * 0.031; 
		pwmwatch[1]=(rc->ch3 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * 0.031;
		PWMA1+=pwmwatch[1];
		PWMA2+=pwmwatch[0];
		pwmwatch[2]=PWMA1;
		pwmwatch[3]=PWMA2;
	VAL_LIMIT(PWMA1,700,1850);
	VAL_LIMIT(PWMA2,560,2140);
	#endif
